          ...
          if (this.spec.antiAlias) {
            for (inty = y1 + 1; inty < y2; ++inty) {
              ...
              if (this.backgroundImage != "") {
                ...
                this.drawPixel(intx+1, inty, pixelcolour, trans, 1, newCorner, false, specRadius);
              }
              else this.drawPixel(intx+1, inty, bcolor, trans >> 1, 1, newCorner, false, specRadius);
            }
            // Draw bar for the border
            if (y3 >= y2) {
              if (y2 == -1) y2 = 0;
              this.drawPixel(intx+1, y2, bcolor, trans, (y3 - y2 + 1), newCorner, false, 0);
            }
            ...
            if (y3 > y1) { // NB condition was >=, changed to avoid zero-height divs
              this.drawPixel(intx+1, (y1 + 1), bcolor, trans, (y3 - y1), newCorner, false, 0);
            }
          }
        }
        else {
