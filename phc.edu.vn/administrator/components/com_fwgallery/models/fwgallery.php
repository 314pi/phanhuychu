...
ORDER BY
    ...
    function remove() {
        if ($cid = (array)JRequest::getVar('cid')) {
            ...
            $this->setError(JText::_('gallery(ies) removed'));
            return true;
    ...
    function publish() {
        if ($cid = (array)JRequest::getVar('cid')) {
            ...
            $image->publish($cid, 1);
            $this->setError(JText::_('gallery(ies) published'));
            return true;
    ...
    function unpublish() {
        if ($cid = (array)JRequest::getVar('cid')) {
            $image = $this->getTable('project');
            $image->publish($cid, 0);
            $this->setError(JText::_('gallery(ies) unpublished'));
            return true;
        } else $this->setError(JText::_('No gallery ID passed to publish'));
        return false;
    }
    ...
    function getGroups() {
		$acl =& JFactory::getACL();
		return array_merge(
			array(JHTML :: _('select.option', '0', JText :: _('Guests'))),
