...
ORDER BY
    ...
    function remove() {
        if ($cid = (array)JRequest::getVar('cid')) {
            ...
            $this->setError(JText::_('image(s) removed'));
            return true;
        }
        $this->setError(JText::_('No image(s) ID passed to remove'));
        return false;
    }
    function publish() {
        if ($cid = (array)JRequest::getVar('cid')) {
            $image = $this->getTable('file');
            $image->publish($cid, 1);
            $this->setError(JText::_('Image(s) published'));
            return true;
        }
        $this->setError(JText::_('No image(s) ID passed to publish'));
        return false;
    }
    function unpublish() {
        if ($cid = (array)JRequest::getVar('cid')) {
            $image = $this->getTable('file');
            $image->publish($cid, 0);
            $this->setError(JText::_('Image(s) unpublished'));
            return true;
        }
        $this->setError(JText::_('No image(s) ID passed to unpublish'));
        return false;
    }
    function select() {
        if ($cid = (array)JRequest::getVar('cid')) {
