...
JToolBarHelper::title(JText::_('Image').'&nbsp;<small>['.JText::_($this->obj->id?'Edit':'New').']</small>', 'fwgallery-image.png');
...
<form action="index.php?option=com_fwgallery&amp;view=files" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data" class="form-validate">
    <fieldset class="adminform">
        ...
        <table class="admintable">
	        ...
	        <tr>
	            <td class="key">
	                <label for="project_id"><?php echo JText::_('Gallery'); ?>:</label>
	            </td>
	            <td>
...
?>
                    <p><?php echo JText :: _('File upload size limit').' '.ini_get('post_max_size'); ?></p>
					<label for="type_id"><?php echo JText :: _('File type') ?>:</label> <?php echo JHTML :: _('select.genericlist', $this->types, 'type_id', '', 'id', 'name', $this->obj->type_id); ?>
...
?>
					<div class="fwg-types" id="fwg-type-1"<?php if ($this->obj->type_id > 1 and $this->obj->_is_type_published) { ?> style="display:none;"<?php } ?>>
		                ...
		                <?php echo JText::_('Filename').':'.$this->obj->filename; ?><br/>
