...
SELECT
	id,
	'.(JFHelper :: isJoomla16()?'title AS ':'').'name
FROM
	#__'.(JFHelper :: isJoomla16()?'usergroups':'core_acl_aro_groups').'
ORDER BY
	lft');
				$groups = array_merge(
					array(JHTML :: _('select.option', '0', JText :: _('Guests'), 'id', 'name')),
        ...
        function getItemid($view='galleries', $id=0, $default=0) {
        	...
        	if ($id and $items = $menu->getItems('link', 'index.php?option=com_fwgallery&view='.$view)) {
        		foreach ($items as $menuItem) if (preg_match('/id\='.$id.'\s/ms', $menuItem->params)) {
        			$item = $menuItem;
        			break;
        		}
        	}
